...
namespace LoRSideTracker
{
    ...
    public partial class GameLogControl : UserControl
    {
        ...
        public void LoadGames(string deckSignature, string deckName, int deckOrdinal, bool hideGamesVsAI)
        {
            Utilities.CallActionSafelyAndWait(this, new Action(() =>
            {
                ...
                if (deckName != null)
                {
                    Games = GameHistory.Games.FindAll(x => deckName == x.MyDeckName && deckOrdinal == x.DeckOrdinal && (!hideGamesVsAI || !x.OpponentIsAI)).ToList();
                }
                else
                {
                    Games = GameHistory.Games.FindAll(x => deckSignature == x.GetDeckSignature() && deckOrdinal == x.DeckOrdinal && (!hideGamesVsAI || !x.OpponentIsAI)).ToList();
                }
